...
public class Hero extends Ship {
    ...
    public void renderGUI(SpriteBatch batch, BitmapFont font) {
        ...
        tmpStr.append("ENGINE: ").append(engineScore).append(" / ").append(engineScoreMax).append("\n");
    ...
    public boolean updateEngine(float dt) {
        float scoreSpeed = engineScore - 1;
        if (Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT) && Gdx.input.isKeyPressed(keysControl.forward)) {
            engineScore -= scoreSpeed * dt * 0.1f;
    ...
    public void consume(PowerUp p) {
        switch (p.getType()) {
            ...
            case MONEY:
                ...
                break;
            case ENGINE:
                engineScore += p.getPower();
                tmpStr.setLength(0);
                tmpStr.append("ENGINE +").append(p.getPower());
                gc.getInfoController().setup(p.getPosition().x, p.getPosition().y, tmpStr, Color.GRAY);
                break;
        }
    }
