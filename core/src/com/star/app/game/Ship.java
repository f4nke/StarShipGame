...
public class Ship {
    ...
    public void accelerate(float dt, float speedMode) {
        if (Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT)) {

            speedMode = 10.0f;
        }

            velocity.x += (float) Math.cos(Math.toRadians(angle)) * enginePower * dt * speedMode;
            velocity.y += (float) Math.sin(Math.toRadians(angle)) * enginePower * dt * speedMode;

            if (hero.getEngineScore() <= 0) {
